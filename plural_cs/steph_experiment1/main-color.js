...
function generateUrnBalls(urnKey, total = 20) {
  ...
  const n_grey = total - n_colored;
  return shuffleArray(Array(n_colored).fill(color).concat(Array(n_grey).fill('lightgrey')));
}
...
timeline.push({
  ...
  pages: [`
    ...
    <div class="instructions-container">
      ...
      Below is an example of one urn. An <b>urn</b> is simply a container that holds a mix of balls. Some balls are <b>colored</b> (e.g., orange, blue, purple, or pink), and others are <b>grey</b>.</p>
...
timeline.push({
  ...
  stimulus: () => `
    <div class="instructions-container">
      ...
      After each trial, you will see whether the result is a <span class="win">win</span> or a <span class="lose">loss</span>.</p>
      <p><b>Please note:</b> Some combinations of draws may appear more than once across trials, as certain urns contain more colored balls than others, making some draw patterns more likely.</p>
    ...
    <div class="center-button">
      <button id="draw-btn" class="jspsych-btn">Draw sample</button>
      <p id="remaining-samples">Remaining samples: 10</p>
      <div id="draw-sample-container" style="display: none;">
        <div class="draw-sample-box">
        <div class='draw-table' id='draw-table'></div>
      </div>
        <div id="feedback-box" style="text-align:center; margin-top:10px;"></div>
      </div>
    <br>${renderUrnsHTML()}
    `,
  choices: "NO_KEYS",
  ...
  on_load: () => {
    let idx = 0;
    const drawTable = document.getElementById("draw-table");
    const drawBtn = document.getElementById("draw-btn");
    const feedbackBox = document.getElementById("feedback-box");

    const headerRow = `<div class='draw-row'>
      ${Object.keys(urnMap).map(urn => `<div class='draw-cell'><b>${urn}</b></div>`).join('')}
      <div class='draw-cell'><b>Result</b></div>
    </div>`;
    drawTable.innerHTML = headerRow;

    drawBtn.onclick = () => {
      if (idx < 10) {
        idx++; 

        const draw = familiarisationDraws[idx - 1];
        if (idx === 1) {
          document.getElementById("draw-sample-container").style.display = "block";
        }

        const result = rule(draw);
        
        jsPsych.data.write({
          questionID: "familiarisation",
          trial_number: idx,
          draw_A: draw.A,
          draw_B: draw.B,
          draw_C: draw.C,
        ...
        let rowHTML = `<div class='draw-row'>`;
        for (const urn of Object.keys(urnMap)) {
          rowHTML += `<div class='draw-cell'><div class="ball" style="background-color:${draw[urn]}"></div></div>`;
        }
        rowHTML += `<div class='draw-cell'><div class="result-box" style="background-color:${result ? 'green' : 'red'}"></div></div>`;
        rowHTML += `</div>`;
        drawTable.innerHTML += rowHTML;

        const winText = result
          ? `<span style="color: green; font-weight: bold;">YOU WIN!</span>`
          : `<span style="color: red; font-weight: bold;">YOU LOSE!</span>`;

        feedbackBox.innerHTML = `${winText}<br>`;

        document.getElementById("remaining-samples").textContent = "Remaining samples: " + (10 - idx);

        if (idx === 10) {
          drawBtn.textContent = "Continue";
          drawBtn.onclick = () => jsPsych.finishTrial();
        }
      }
    };
  }
    ...
    `;
      ...
      const drawTable = `
        <div class='draw-table'>
          <div class='draw-row'>
            ${Object.keys(draw).map(urn => `<div class='draw-cell'><b>${urn}</b></div>`).join('')}
            <div class='draw-cell'><b>Result</b></div>
          </div>
          <div class='draw-row'>
            ${Object.keys(draw).map(urn => `<div class='draw-cell'><div class="ball" style="background-color:${draw[urn]}"></div></div>`).join('')}
            <div class='draw-cell'><div class="result-box" style="background-color:${rule(draw) ? 'green' : 'red'}"></div></div>
          </div>
        </div>
      `;
